...
class TwoLayerNet(object):
    ...
    def loss(self, X, y=None, reg=0.0):
        ...
        loss = np.mean(-scores[np.arange(N), y] + np.log(np.sum(np.exp(scores), axis=1)))
        ...
        grads['W2'] = np.matmul(relu.T, softmax)/N + reg * 2.0 * W2 
        grads['b2'] = (np.matmul(np.ones(N), softmax))/N

        dLdrelu = np.matmul(W2, softmax.T)
        dLdrelu[fc1.T < 0] = 0.0
        grads['W1'] = np.matmul(dLdrelu, X).T / N  + reg * 2.0 * W1
        grads['b1'] = np.matmul(dLdrelu, np.ones(N))/N
