...
class ThreeLayerConvNet(object):
    ...
    def __init__(
        ...
        C, H, W = input_dim
        # can we assume stride = 2? 
        H_prime = 1 + int((H - 2) / 2)
        W_prime = 1 + int((W - 2) / 2)
        self.params['W1'] = np.random.normal(0, weight_scale, 
          (num_filters, C, filter_size, filter_size))
        self.params['W2'] = np.random.normal(0, weight_scale, (num_filters*H_prime*W_prime, hidden_dim))
    ...
    def loss(self, X, y=None):
        ...
        out1, cache1 = conv_relu_pool_forward(X, self.params['W1'], 
